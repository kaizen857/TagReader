...
- Parser 共用 `ReadContext::input` 和 `AVFormatContext`；二进制读取用绝对 offset + `ReadRange()`，避免依赖或污染 stream 状态。
...
- 常用运行：`./build/TagReaderTest <audio-file-path> [cover-export-dir]`；`./build/TagReaderSecuritySmoke <cover-export-dir> <audio-file-path> [...]`；`./build/TagReaderRegressionTests --list|<TR-AUDIT-case-id>`（当前 `TR-AUDIT-001` 到 `TR-AUDIT-031` 都已实现）。
