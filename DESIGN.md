...
- 封面块应从音频文件原始字节中按对应格式直接提取，不能通过 FFmpeg metadata 字典获取。
- 封面导出应支持音频文件中存储的封面为任意可解码图像格式，不能只支持 PNG/JPEG。
- 对外导出的封面文件格式必须统一为 PNG；无论源封面是 PNG、JPEG、BMP、WEBP、GIF、TIFF 或其他格式，导出前都应完成图像解码并重新编码为 PNG。
- 封面转码过程可以使用 FFmpeg 或 OpenCV 等图像解码/编码能力；在满足正确性的前提下，以最快处理速度优先。
- 如果源封面图像无法被所选图像后端解码，才允许跳过封面或保持封面路径为空。
