...
- `id3`：ID3v2.2/v2.3/v2.4 dispatch、未知大帧跳过、ID3v2.4 tag/frame unsync、footer/extended header、APIC unsync，以及歌词行数和 LRC timestamp DoS 样本。
...
- `ogg`：Ogg Vorbis comment page、截断 page、异常 continuation、payload 边界截断/超限，以及 Vorbis 非法 entry 后续字段隔离。
- `mp4`：MP4 atom tree、截断 atom、深嵌套 atom、multi-data item、meta payload/version 异常、extended atom、嵌套 item、`covr` 无效后有效数据。
